...
from .snp import PyramidLayer, MultiScaleModule
...
class STBlock(nn.Module):
    def __init__(
        ...
        # Extract Global Temporal Feature by LSTM
        self.T_Global = CustomLSTM(embed_size, embed_size)
...
class MVSTT(nn.Module):
    ...
    def forward(self, x):
        # x: [batch_size, time_step, n] [16, 12, 307]
        # x [batch_size, n, time_step]  [16, 307, 12]
        # torch.Size([16, 1, 307, 12])
        x = x.permute(0, 2, 1).unsqueeze(1)
        input_Transformer = self.conv1(x) # torch.Size([16, 64, 307, 12])
        input_Transformer = input_Transformer.permute(0, 2, 3, 1) # torch.Size([16, 307, 12, 64])
